#!/usr/bin/env python3
"""
side_by_side_timeline.py

Compose N portrait MP4 videos side-by-side on a shared timeline using FFmpeg.
(No frame-by-frame processing in Python; a single FFmpeg filtergraph does the work.)

START SEMANTICS (IMPORTANT)
--------------------------
--start_mode sync (DEFAULT, requested behavior):
  Each --start is the timestamp (seconds) INSIDE that clip where all clips should be in sync.
  The output timeline is constructed so that at a single moment T_sync, every clip is at its
  own sync timestamp.

  Math:
    At sync moment T_sync, we want: (T_sync - timeline_start_i) == sync_i
    Choose T_sync = max(sync_i) so no timeline_start_i is negative.
    Therefore:
      timeline_start_i = max(sync_times) - sync_i

  Example:
    video1 sync=4.5, video2 sync=5.5
    max=5.5 => start1=1.0, start2=0.0  (video2 starts 1s before video1)

--start_mode timeline (legacy behavior):
  Each --start is directly the timeline time when playback begins.

Timeline rules (after converting to timeline_start_i):
- Timeline starts at t = 0.
- Before each video's start time, show a FREEZE of its first frame (not black).
- At its start time, that video begins playback from timestamp 0 of that input.
- Output ends at:
    total_duration = max_i ( timeline_start_i + duration(video_i) )

Layout:
- Left-to-right order is the order you pass --video arguments
- Each side scaled to --height (preserving aspect ratio), then stacked horizontally.

Labels:
- Optional per side (bottom-center within that side only).
- Drawn only if label is non-empty.

Audio:
- none (default): no audio track
- videoN: audio from input N (1-based), delayed so it starts at timeline_start_N
- mix: mix all available audio tracks, each delayed by its own timeline start
- Missing audio streams are handled gracefully (warn + proceed).
"""
...
def compute_timeline_starts(starts: List[float], start_mode: str) -> Tuple[List[float], float]:
    """
    Convert user-provided starts into timeline start offsets.

    start_mode="timeline": starts are already timeline offsets.
    start_mode="sync": starts are in-clip sync timestamps; timeline starts are computed as:
        timeline_start_i = max(starts) - starts[i]

    Returns:
      (timeline_starts, t_sync)
        - timeline_starts are >= 0
        - t_sync is the timeline time where all clips align (in sync mode), else 0
    """
...
def build_filter_complex(
    ...
    if mode == "single":
        ...
        if not has_audio[single_idx]:
            eprint(f"Warning: requested audio from video{single_idx + 1}, but it has no audio stream. Output will be silent.")
            return ";".join(parts), False
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    # DEFAULT is now "sync" with your intended meaning (timestamps inside each clip where they align)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        # If in sync mode, the sync timestamps must be within the clip durations.
        if args.start_mode == "sync":
            for i, (sync_t, info) in enumerate(zip(args.starts, infos), start=1):
                # allow a small epsilon for container rounding
        ...
        if args.start_mode == "sync":
            print(f"Sync moment occurs at timeline t={t_sync:.3f}s. Timeline starts: {', '.join(f'{x:.3f}' for x in timeline_starts)}")
        return 0
...
# --- PyCharm-friendly launcher (supports N videos) ----------------------------
...
def _build_args_interactively() -> List[str]:
...
if __name__ == "__main__":
    ...
    if INTERACTIVE_PROMPT:
        raise SystemExit(main(_build_args_interactively()))

    raise SystemExit(main(PYCHARM_DEFAULTS))
# -----------------------------------------------------------------------------
